...
def process(state : State):
    ...
    if state['age'] < 18:
       print('Age check is successful')
       approval = interrupt(f"Do you want to approve the admission of {state['age']} years old?")
       if approval:
...
initial_state = State(
    messages=[HumanMessage(content="Process admission of Scott Bentton who is 15 years old using the tool")]
)
